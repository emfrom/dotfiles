...
C-u is not on the home row and it's awkward to hit fast
C-: is not is any keymap (at the time of writing) at all

Bug: It doesn't work for repeated presses, why?

#+begin_src emacs-lisp

  ;;
  ;; Bind C-: to universal argument
  ;;
  (global-set-key (kbd "C-:") #'universal-argument)
  
#+end_src
**** Find keybindings
Find all maps that bind a key 
#+begin_src emacs-lisp
  (defun emf/find-key-in-all-maps (key)
    "Prompt for a KEY and list all keymaps that bind it, neatly aligned."
    (interactive "kFind key in all maps: ")
    (let (results)
      (mapatoms
       (lambda (sym)
         (when (and (boundp sym)
                    (keymapp (symbol-value sym)))
           (let ((cmd (lookup-key (symbol-value sym) key)))
             (when (and cmd (commandp cmd))
               (push (cons (symbol-name sym) (symbol-name cmd)) results))))))
      (with-current-buffer (get-buffer-create "*Key Search*")
        (erase-buffer)
        (insert (format "%-40s %s\n" "Keymap" "Command"))
        (insert (make-string 60 ?-) "\n")
        (dolist (pair (sort results (lambda (a b) (string< (car a) (car b)))))
          (insert (format "%-40s %s\n" (car pair) (cdr pair))))
        (display-buffer (current-buffer)))))

  ;; ;; Override default help key 
  ;; (global-set-key (kbd "C-h K") #'emf/find-key-in-all-maps)
...
#+begin_src emacs-lisp
  ...
  ;; Disable GUI elements
  ;;
  (defun emf/gui-setup (frame)
    "Disable GUI elements for each new FRAME."
    (with-selected-frame frame
      (progn (when (display-graphic-p frame)
  ...
  (add-hook 'after-make-frame-functions 'emf/gui-setup)

  (emf/gui-setup (selected-frame))
...
#+begin_src emacs-lisp
    ...
    (use-package gptel
      ...
      (setq gptel-default-model 'gpt-5-nano))
...
#+begin_src emacs-lisp
 ...
 (bind-key "C-c t" (lambda () (interactive) (ansi-term "/usr/bin/env bash")))
...
#+begin_src emacs-lisp
  ...
  (defvar emf/recentf-save-timer nil
    "Timer for debounced saving of recentf list.")

  (defun emf/debouncedf-save ()
    "Debounced version of `recentf-save-list`, suppressing messages."
    (when emf/recentf-save-timer
      (cancel-timer emf/recentf-save-timer))
    (setq emf/recentf-save-timer
          (run-with-timer
           2 nil
           (lambda ()
             (let ((inhibit-message t))
...
(setq recentf-exclude
      '("\\/org-mode\\/roam\\/"    ;; match any path containing /org-mode/roam/
        "\\.cache/"                ;; example: exclude cache files
        "/tmp/"                    ;; exclude temp files
        "^/sudo:"                  ;; exclude sudo/tramp files
        ))
  ...
  ;; Advice using lambda to absorb args
  (advice-add 'recentf-add-file :after (lambda (&rest _) (emf/debouncedf-save)))
...
#+begin_src emacs-lisp
  ...
  ;; Use pdf-view-mode for PDF files
  (add-to-list 'auto-mode-alist '("\\.pdf\\'" . pdf-view-mode))

  ;; Disable line numbers in pdf-view-mode buffers
...
(defun emf/running-in-termux-p ()
  "Return non-nil if Emacs is running inside Termux."
  (and (getenv "PREFIX")
       (string-prefix-p "/data/data/com.termux/" (or (getenv "HOME") ""))))
...
#+begin_src emacs-lisp
    ...
    (use-package org
      ...
      (setq org-startup-with-inline-images t) ; Enables inline images
      (org-display-inline-images) ; Displays images inline
...
TODO: Fix conflict with completions 
...
#+begin_src emacs-lisp
      ...
      ;;
      ;; Automatic wiki links in roam docs
      ;; 
      (defvar emf/org-roam-autowiki-prompt
        (concat
         "This file has a title and tags on the form:\n"
         "#+title: xxx (yyy)\n\n"
         "Where xxx is the tile and yyy are tags defining the area of focus of the title.\n\n"
         "Do a proper search on wikipedia and see if there is an article that matches this exact title in the correct focus area. "
         "Review the content of the wikipedia article and see if it matches what is written in this arcticle.\n\n"
         "If you find such an article reply with the link, if not do find one not reply anything.\n"
         "Very important, do not reply anything other than the link or nothing.\n"
         "No commentary, thinking process or anything else. Just the link, or nothing.\n")
        "LLM prompt used to automatically add Wikipedia links to Org-roam files.")

      (defun emf/org-roam-autowiki ()
      "Ask an LLM for a relevant Wikipedia link and insert it under the '* Links' heading."
      (interactive)
      (if (save-excursion
            (goto-char (point-min))
            (re-search-forward "^\\* Links\\b" nil t))
          (let* ((buf (current-buffer))
                 (prompt emf/org-roam-autowiki-prompt)
                 (input (concat
            ...
            (gptel-request
             ...
             (lambda (response _info)
               (with-current-buffer buf
                 (save-excursion
                   (goto-char (point-min))
                   (when (re-search-forward "^\\* Links\\b" nil t)
                     (forward-line 1)
                     ;; Skip drawers
                     (while (looking-at "^:") (forward-line 1))
                     (insert "\n" (string-trim response) "\n")
                     (save-buffer)))))))
        (message "No Links heading in buffer")))
      
    (with-eval-after-load 'org
      (define-key org-mode-map (kbd "C-c n w") #'emf/org-roam-autowiki))
...
#+begin_src emacs-lisp
  ...
  ;;
  ;;
  ;; Use LLM to generate org-drill questions inside a roam entry
  ;;
  ;;
  (defvar emf/org-drill-prompt
    (concat
     "Under the '* Flashcards' heading at the end of this org-roam file, generate 1–3 new spaced repetition entries.\n\n"
     "Each entry must consist of a short, factual question and its correct answer, based strictly on the text above the '* Flashcards' heading.\n\n"
     "Format each entry exactly as follows:\n\n"
     "** Question :drill:\n"
     "<<insert question here>>\n\n"
     "*** Answer\n"
     "<<insert answer here>>\n\n"
     "Do not include the '* Flashcards' heading itself.\n"
     "Do not repeat or paraphrase existing questions already under that heading.\n"
     "Do not add any explanations, commentary, symbols, or extra formatting.\n"
     "Do not make any questions about document metadata like creation date, author or the like. Only make questions based on content."
     "Only return properly formatted org-drill entries in the structure shown above.\n\n"
     "RETURN NO OTHER TEXT OR SYMBOLS THAN CORRECTLY FORMATTED QUIESTIONS\n"
     "DO NOT DEVIATE FROM THE PROSCRIBED FORMAT\n")
    "Prompt used to generate org-drill questions via.")


  (defun emf/org-drill-autoquestions ()
    "Generate and append org-drill questions under the '* Flashcards' heading."
    (interactive)
    (if (save-excursion
          (goto-char (point-min))
          (re-search-forward "^\\* Flashcards\\b" nil t))
        (let* ((buf (current-buffer))
               (prompt emf/org-drill-prompt)
               (input (concat
                       (buffer-substring-no-properties (point-min) (point-max))
                       "\n\n" prompt)))
          (gptel-request
            ...
            (lambda (response _info)
              (with-current-buffer buf
                (save-excursion
                  (goto-char (point-min))
                  (when (re-search-forward "^\\* Flashcards\\b" nil t)
                    (forward-line 1)
                    ;; Skip drawers
                    (while (looking-at "^:") (forward-line 1))
                    (insert "\n" (string-trim response) "\n")
  ...
  (defun emf/org-roam-autoeverything ()
    (interactive)
    (progn
      (emf/org-drill-autoquestions)
      (emf/org-roam-autowiki)))

  (define-key org-mode-map (kbd "C-c n d") #'emf/org-drill-autoquestions)
  (define-key org-mode-map (kbd "C-c n a") #'emf/org-roam-autoeverything)
#+end_src
...
#+begin_src emacs-lisp
  ...
  (add-to-list 'gnus-secondary-select-methods
             '(nnmbox "local-mail"
                      (nnmbox-mbox-file "/var/mail/emfrom")))
...
#+begin_src emacs-lisp
  ...
  (use-package paredit
    ...
    :bind (:map paredit-mode-map
  	      ("C-<left>"  . nil)                         ; restore word navigation
  	      ("C-<right>" . nil)
...
#+begin_src emacs-lisp
  ...
  (defun emf/lsp-tab-handler ()
    "Complete selection if company tooltip is visible, otherwise indent or complete."
    (interactive)
...
#+begin_src emacs-lisp
    (defun emf/cc-style ()
      ...
      (setq c-cleanup-list (remove 'comment-intro c-cleanup-list)))


    (add-hook 'c-mode-hook #'emf/cc-style)

  ;;  (defun emf/c-hide-tests ()
  ;;   (interactive)
  ;;   (hs-minor-mode 1)
  ;;   (save-excursion
  ;;     (goto-char (point-min))
  ...
  ;; (add-hook 'c-mode-hook  'emf/c-hide-tests)
#+end_src 
...
#+begin_src emacs-lisp
  ...
  ;; Emacsclient server
  ;;
  (unless (emf/running-in-termux-p)
    (progn
      (require 'server)
...
#+begin_src shell :tangle ./exwm/start-exwm.sh :mkdirp yes
  ...
  #
  [[ -z XDG_CONFIG_HOME ]] && export XDG_CONFIG_HOME=$HOME/etc

  #
  # Load enviornment
  #
  [[ -f $XDG_CONFIG_HOME/env/load-env.sh ]] && source $XDG_CONFIG_HOME/env/load-env.sh
