...
There's something that doesnt work with exwm and the server so I dont start the server in the X emacs
...
This seems very strange, like I can run the server and connect to it but it's like it's out of asyncrounous mode some how. I have to switch to exwm to get the server to update
