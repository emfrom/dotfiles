...
Credit is given here and there.

Very bit thanks to David Wilson that got me back into using emacs, almost feels like it's the 90s again.

and thanks to all others who have forged the way! Onwards into the breach!
...
#+begin_src emacs-lisp
  ...
  ;;
  (setq package-gnupghome-dir "~/emacs/elpa/gnupg")
...
I use a list of directories so I can change it by addint the following to =.dir-locals.el=
  ((org-mode . ((eval . (add-to-list 'emfrom/org-babel-tangle-dirs (file-name-directory (or load-file-name buffer-file-name)))))))

#+begin_src emacs-lisp

  ;; Define a customizable list for directories to tangle automatically
  (defvar emfrom/org-babel-autotangle-dirs
    (mapcar #'expand-file-name '("~/etc/emacs/" "~/etc/bash/"))
...
#+end_src
**** TODO Fix org mode agenda so it doesnt take to lone
