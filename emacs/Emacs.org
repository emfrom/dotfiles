...
#+begin_src emacs-lisp
  ...
  ;; 
  (dolist (file (directory-files "~/etc/env" t))
...
Added the emf/start-org-drill and set org-drill-scope to directory,
opening a file in the roam directory before calling org-drill this way solves the cache error
...
#+begin_src emacs-lisp
  ...
  (use-package org-drill
    ...
    ;; Fixing the cache error
    (setq org-drill-scope 'directory)
    )    

  ;; Function that fixes the cache error
  ;; The opening of the file is the key to fixing the cache bug,
  ;;  done any other way, drill can throw a cache error
  ;; (org-drill is old and unmaintained)
  (defun emf/start-org-drill ()
    "Open the org-drill file in a single window and start org-drill."
    (interactive)
    (let ((org-drill-file "~/opt/org-mode/roam/20241030154050-org_drill.org"))
      (find-file org-drill-file)
      (delete-other-windows)  
      (org-drill)))           

  (global-set-key (kbd "C-c d") 'emf/start-org-drill)
#+end_src
...
#+begin_src emacs-lisp
  ...
  ;; Set the rust env-variables
  (let ((env-file "/home/emfrom/etc/env/rust"))
