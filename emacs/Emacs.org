...
#+begin_src emacs-lisp
  ...
  (use-package haskell-mode
    ...
    (defun haskell-setup-repl-window ()
      "Open or focus Haskell REPL in a window to the right of the source."
      (interactive)
      (let ((repl-buffer (haskell-session-interactive-buffer (haskell-session))))
        (unless (get-buffer-window repl-buffer)
  	(split-window-right))
        (let ((repl-window (display-buffer-in-side-window repl-buffer '((side . right)))))
      ...
      :bind (:map haskell-mode-map
  		...
  		("C-c h s" . emf/haskell-save-and-reload)
  		("C-c h r" . emf/haskell-setup-repl-window))))
